...
_PALEO_TASK_SYSTEM = (
    ...
    "NOVELTY TARGETING (critical): AVOID textbook taphonomic patterns that any "
    "paleontologist would recognise — these will be filtered by the Gate 0 "
    "question prescreen. KNOWN/TEXTBOOK (do NOT propose): 'deep-water preserves "
    "more fossils than peritidal'; 'diversity increases through the Phanerozoic'; "
    "'carbonate vs clastic preservation differs'; 'younger rocks have more fossils' "
    "(Pull of the Recent); 'environment X has higher diversity than Y' (standard "
    "taphonomic bias). INSTEAD, aim for GENUINELY UNDER-STUDIED questions:\n"
    "  * LATITUDINAL diversity gradients in deep time (does the modern-style LDG "
    "exist in the Proterozoic or early Paleozoic, and is its strength age-dependent?)\n"
    "  * DIVERSITY-ATMOSPHERE coupling (does occurrence density shift across the "
    "GOE ~2400 Ma, Snowball Earth ~720 Ma, or Cambrian explosion ~540 Ma in a way "
    "that suggests atmospheric forcing?)\n"
    "  * THRESHOLD / tipping-point dynamics (is there an age or environment beyond "
    "which occurrence density changes regime — not a monotonic trend but a STEP?)\n"
    "  * COUNTERINTUITIVE patterns (an environment that SHOULD have low diversity "
    "but doesn't, or a time interval with unexpected diversity)\n"
    "  * UNDER-SAMPLED intervals (the Mesoproterozoic 1600-1000 Ma is notoriously "
    "under-studied — does the PBDB data reveal a pattern there?)\n"
    "  * INTERACTION effects (does the environment-diversity relationship CHANGE "
    "across age — i.e. is the preservation bias itself age-dependent?)\n"
