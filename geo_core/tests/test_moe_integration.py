...
from geo_core.causal.routing import (
    MoECapabilityRouter,
...
def test_capability_imports(results: IntegrationTestResults):
    ...
    import_tests = [
        # Causal
        ("geo_core.causal.model.scm", "StructuralCausalModel"),
        ("geo_core.causal.discovery.pc_algorithm", "PCAlgorithm"),

        # Memory
        ("geo_core.memory.episodic.memory", "EpisodicMemory"),
        ("geo_core.memory.working.memory", "WorkingMemory"),
        ("geo_core.memory.semantic.memory", "SemanticMemory"),

        # Simulation
        ("geo_core.simulation.physics.simulator", "PhysicsSimulator"),

        # Metacognitive
        ("geo_core.metacognitive.monitoring", "CognitiveMonitor"),
    ]
...
def test_unified_system_integration(results: IntegrationTestResults):
    ...
    try:
        ...
        for mode in modes:
            try:
                system = create_geo_stan_system(mode=mode)
        ...
        results.add_fail("Import create_geo_stan_system", str(e))
...
def test_permissionless_hooks(results: IntegrationTestResults):
    ...
    try:
        from geo_core.causal.routing.moe_router import Expert
