...
elif 'geo_core' in str(current_dir):
...
def test_core_imports():
    ...
    modules = [
        ...
        'geo_core.causal',
        'geo_core.memory',
        'geo_core.intelligence',
        'geo_core.capabilities',
        'geo_core.scientific_discovery',
    ]
...
def test_v47_causal_imports():
    ...
    from geo_core.causal.discovery import (
    ...
    print("  ✓ geo_core.causal.discovery imports")

    # Test from inference subdirectory
    from geo_core.causal.inference import (
        SimulationBasedInferenceEngine,
        SBIMethod,
        create_sbi_engine,
    )
    print("  ✓ geo_core.causal.inference imports")

    # Test from top-level causal
    from geo_core.causal import (
        BayesianStructureLearner,
        ExpectedInformationGainCalculator,
        OnlineCausalLearner,
        SimulationBasedInferenceEngine,
    )
    print("  ✓ geo_core.causal top-level imports")
...
def test_v47_metalearning_imports():
    ...
    try:
        from geo_core.reasoning.maml_optimizer import (
            MAMLOptimizer,
...
def test_causal_physics_integration():
    ...
    from geo_core.causal import StructuralCausalModel
    from geo_core.physics import PhysicsConstraint
...
def test_metalearning_domain_integration():
    ...
    try:
        from geo_core.reasoning.maml_optimizer import MAMLOptimizer
...
def test_domain_orchestrator():
    ...
    from geo_core.domains import DomainRegistry

    registry = DomainRegistry()

    # Try to register all available domains
    domains_to_register = [
        ('ism', lambda: __import__('geo_core.domains.ism', fromlist=['create_ism_domain']).create_ism_domain()),
        ('star_formation', lambda: __import__('geo_core.domains.star_formation', fromlist=['create_star_formation_domain']).create_star_formation_domain()),
        ('high_energy', lambda: __import__('geo_core.domains.high_energy', fromlist=['create_high_energy_domain']).create_high_energy_domain()),
    ]
...
def test_unified_orchestrator():
    ...
    try:
        from geo_core.core.unified import create_unified_stan_system
        print("  ✓ create_unified_stan_system imported")
    except ImportError as e:
        print(f"  ⚠ Unified system not available (expected in degraded mode): {e}")

    try:
        from geo_core.core.unified_enhanced import create_geo_stan_system
        system = create_geo_stan_system()
        print("  ✓ create_geo_stan_system works")
    except Exception as e:
...
def test_causal_graceful_degradation():
    ...
    from geo_core.causal.discovery.bayesian_structure_learning import (
        BayesianStructureLearner,
        InferenceMethod,
    )
...
def test_active_discovery_pipeline():
    ...
    from geo_core.causal.discovery.eig_calculator import (
        create_eig_calculator, ObservationPlan, NoiseModel
    )
    from geo_core.causal.discovery.online_causal_learning import (
