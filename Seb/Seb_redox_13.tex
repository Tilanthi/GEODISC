...
definitive test. The most direct test is kinetic: using the \citet{Rimstidt1980} rate law under
...
The compilation supports the hypothesis in direction: across 13 biotas of accepted biogenicity,
silicification and fidelity scores show a consistent positive association (Spearman $\rho = 0.87$,
$p < 0.001$), though both scores derive from the same descriptive literature and the statistics
should not be read as a confirmatory test. GOE position appears to retain some explanatory power
after accounting for silicification, but the magnitude cannot be resolved at this sample size. The
hypothesis is supported but not confirmed; the GOE-associated residual's partition between biology
...
rank coefficient. Because both scores derive from the same descriptive literature, the statistics
characterise the consistency of the published consensus rather than providing a confirmatory test;
this limitation is restated in the Discussion, where it bounds the strength of the claim.
...
The results are organised so that the most direct line of evidence --- the kinetic calculation,
the most directly testable element of the argument --- is presented first, followed by the
compilation-based statistics. The systematic compilation itself (Table~\ref{tab:panel}) is the
substrate for the statistical summary that follows.
...
introduces large uncertainty in the effective rate constant. The inset perturbation histogram
addresses coding noise but not the shared-provenance circularity discussed in the text.}
\label{fig:kinetic}
\end{figure}

\subsection{Compilation statistics}

Across the 13 accepted-biogenicity biotas, silicification and fidelity scores show a consistent
positive association (Spearman $\rho = 0.87$). A permutation test (100{,}000 random reassignments)
confirms this is stronger than expected by chance ($p < 0.001$). However, these statistics should not
be read as a confirmatory test: both scores are derived from the same descriptive literature, written
by overlapping research groups, and the association may partly reflect shared descriptive provenance
rather than two independently measured properties. The robustness checks (permutation, null-model,
perturbation) address sampling noise and coding subjectivity, but they do not address this systematic
circularity, which is a different failure mode. The compilation is best understood as a structured
summary of a consistent pattern in the published consensus, not as an independent empirical test.
Including the two contested-biogenicity biotas (Apex, Dresser; $n = 15$) leaves the association
essentially unchanged ($\rho = 0.87$). GOE position shows a weaker positive association with
fidelity that persists in direction after accounting for silicification, though its magnitude cannot
be resolved at this sample size; the residual could reflect genuine biological diversification, an
unmeasured preservational variable correlated with the GOE, or sampling bias, and the compilation
cannot distinguish among these alternatives.
...
The compilation is consistent with the hypothesis that silicification is a dominant predictor of
cellular fidelity across the Precambrian microfossil record ($\rho = 0.87$, $p < 0.001$), but a
GOE-associated residual signal in fidelity persists after accounting for silicification, though its
magnitude cannot be resolved at this sample size. The defensible position is therefore narrower than a
...
(principally Lepot, Javaux and Wacey). The within-Gunflint iron covariation is single-locality,
single-element, possibly biogenic, and confounded by wall recalcitrance. None of these limitations
undermines the principal observation --- that the silicification--fidelity association is robust in
direction --- but they do bound the strength of the claim about the residual GOE signal, whose
biological versus preservational origin the compilation cannot decide.

The `supported but not confirmed' framing applies with particular force to the circularity concern:
the robustness checks confirm that the observed association is stable to random coding noise, but
they cannot exclude the possibility that both scores are partly labels for the same underlying
descriptive judgement. Only blind, cross-group rescoring --- in which silicification is coded from
mineralogical observations and fidelity from palaeobiological observations, by separate research
groups unaware of each other's scores --- can resolve this.
...
microfossils. The hypothesis is supported, but not confirmed. The most direct test is kinetic:
...
from surface catalysis and inhibition. The compilation is concordant in direction: across 13 biotas
of accepted biogenicity, silicification and fidelity scores show a consistent positive association
(Spearman $\rho = 0.87$, $p < 0.001$). These statistics should not be read as a confirmatory test:
both scores are derived from the same descriptive literature, written by overlapping research groups,
and the association may partly reflect shared descriptive provenance rather than two independently
measured properties. The observed $\rho$ should be treated as an upper bound rather than a point
estimate. GOE position appears to retain some explanatory power after accounting for silicification,
but the magnitude cannot be resolved at this sample size. Thermal maturity is controlled by design:
all 13 formations are of low metamorphic grade and cannot vary enough to explain the fidelity range.
Whether the GOE residual reflects genuine biological diversification, an unmeasured preservational
change, or sampling bias remains unresolved by the compilation. The biomarker evidence for pre-GOE
eukaryotes is contaminated, but the obligate aerobicity of sterol biosynthesis gives eukaryotes a
mechanistically independent reason to diversify with oxygen --- one candidate biological interpretation
of the residual. We state the two conditions that would falsify the silicification hypothesis and
specify the experimental and analytical programme needed to test them, one element of which is
achievable on existing material. Confirmation awaits that programme; the present paper makes the case
that the hypothesis is worth testing.
