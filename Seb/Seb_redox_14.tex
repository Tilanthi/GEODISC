...
cell on the order of hours --- one to two orders of magnitude faster than anoxic decay, which
proceeds over days to weeks \citep{TaxonRedox2025}. The compilation supports the hypothesis in
direction: across 13 biotas of accepted biogenicity, silicification and cellular fidelity scores
show a consistent positive association across the compilation, though both scores derive from the
same descriptive literature and the statistics should not be read as a confirmatory test. The
hypothesis is supported but not confirmed; whether any residual GOE-associated variation in fidelity
reflects biology or an unmeasured preservational variable remains unresolved.
\end{abstract}
...
magnitude shorter than the half-life of cellular ultrastructure under anoxic decay, which is on the
order of days to weeks \citep{TaxonRedox2025}. The kinetic calculation therefore establishes that entombment is
...
one to two orders of magnitude faster than anoxic decay over days to weeks \citep{TaxonRedox2025}. Surface catalysis or inhibition
...
circularity, which is a different failure mode. The perturbation analysis (Fig.~1B) was generated by
applying $\pm$1 random noise to each of the 26 Table~1 scores over 10{,}000 trials, using a
deterministic random seed. The compilation is best understood as a structured
...
essentially unchanged ($\rho = 0.87$). The compilation cannot distinguish whether any remaining
variation in fidelity reflects biological diversification, an unmeasured preservational variable, or
sampling bias; this question is taken up in the Discussion.
...
cellular fidelity across the Precambrian microfossil record ($\rho = 0.87$, $p < 0.001$). GOE
position may retain some explanatory power not captured by the silicification score, though the
...
silicification on which cellular fidelity primarily depends. The origin of any such GOE-associated
variation --- whether biological diversification, an unmeasured preservational change, or sampling
bias --- cannot be resolved from this compilation alone.
...
magnitude faster than anoxic decay over days to weeks \citep{TaxonRedox2025}, establishing feasibility with quantified uncertainty
...
estimate. Thermal maturity is controlled by design: all 13 formations are of low metamorphic grade
and cannot vary enough to explain the fidelity range. Whether any residual variation in fidelity
reflects genuine biological diversification, an unmeasured preservational change, or sampling bias
remains unresolved by the compilation. The biomarker evidence for pre-GOE
...
All data are compiled from previously published sources cited in Table~1. The scoring protocol,
permutation and perturbation routines, and kinetic calculation code are available from the
...
\bibitem[Taxon-specific redox(2025)]{TaxonRedox2025} (2025). Taxon-specific redox conditions control
fossilisation pathways. \textit{Nature Communications} 16, 59372.
