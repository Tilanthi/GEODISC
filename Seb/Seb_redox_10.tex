% v10 --- MAJOR REVISION. Adds three genuine quantitative results: (a) permutation-validated
% statistics (p = 0.0006), (b) perturbation-robustness analysis (+/-1 coding noise, positive in
% 95.2% of 10,000 trials), and (c) an explicit kinetic calculation (Rimstidt & Barnes 1980 rate
% law, 100 nm layer in ~1.3 h vs. 3-14 d decay). The silicification-fidelity rho = 0.87 is now
% framed as an upper bound; direction robust, magnitude inflated by clean coding.
...
\begin{abstract}
\noindent We systematically compiled published petrographic data for 15 Precambrian chert-hosted
microfossil biotas (13 of accepted biogenicity and 2 contested) to test whether early mineral
entombment, predominantly silicification, accounts for the apparent diversification of
morphologically diagnostic microfossils across the Great Oxidation Event (GOE; $\sim$2.4~Ga).
Silicification is the strongest single predictor of cellular fidelity (Spearman $\rho = 0.87$,
$p < 0.001$, $n = 13$); a permutation test ($p = 0.0006$) and a $\pm$1-point perturbation analysis
(positive in 95.2\% of 10{,}000 trials) confirm that the \emph{direction} of the association is
robust to both sampling and coding subjectivity, though the observed magnitude should be treated as
an upper bound. GOE position retains independent explanatory power after controlling for
silicification (partial $r = 0.72$, $p = 0.006$), indicating a residual GOE-associated signal in
fidelity whose origin the compilation alone cannot resolve. An explicit kinetic calculation, using
the \citet{Rimstidt1980} silica-precipitation rate law under Precambrian silica saturation
($\Omega \approx 1.8$), shows that a 100~nm silica layer deposits on a microbial cell in
$\approx$1.3~hours --- one to two orders of magnitude faster than anoxic decay (3--14 days) ---
establishing that early entombment is kinetically feasible, with quantified uncertainty from surface
catalysis and inhibition. The biomarker evidence for pre-GOE eukaryotes is contaminated, but the
obligate aerobicity of sterol biosynthesis gives eukaryotes a mechanistically independent reason to
diversify with oxygen. We conclude that silicification is a dominant taphonomic confound in the
post-GOE microfossil record, that the direction of the silicification--fidelity association is
robust but its magnitude is an upper bound, and that a significant GOE-associated residual persists
whose partition between biology and preservation remains unresolved.
\end{abstract}
...
record. We compile silicification and cellular-fidelity scores for 15 Precambrian chert-hosted
microfossil biotas and use rank-correlation and partial-correlation statistics to ask two questions:
does silicification predict cellular fidelity across the record, and does GOE position retain
explanatory power once silicification is controlled for? A kinetic calculation then assesses whether
catalysed silica entombment can outpace decay. The compilation is semi-quantitative and drawn from a
literature dominated by a few research groups, a limitation stated explicitly below.
...
Gunflint Fm ($\sim$1.88~Ga; post) & Accepted & 3 & 3 & \citep{Lepot2017a,Wacey2013,Alleon2016} \\
...
The systematic compilation (Table~\ref{tab:panel}; Fig.~\ref{fig:meta}) yields four principal
findings.
...
partial Spearman correlation between GOE position (coded pre/spanning/post) and fidelity,
controlling for silicification score, is partial $r = 0.716$ ($p = 0.0059$). GOE position therefore
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\linewidth]{fig_v9_meta.png}
\caption{Systematic compilation of 15 Precambrian chert-hosted microfossil biotas. Silicification
...
including contested). GOE position retains significant explanatory power after controlling for
silicification (partial $r = 0.72$, $p = 0.006$), indicating a residual GOE-associated signal in
fidelity whose origin cannot be resolved from this compilation. Crosses = contested biogenicity
(Apex, Dresser), included for sensitivity but excluded from the primary analysis.}
\label{fig:meta}
\end{figure}
...
experimental taphonomy). However, the effective rate constant on microbial surfaces is uncertain by
orders of magnitude: cell wall functional groups catalyse silica nucleation (enhancing the rate),
while organic coatings can inhibit precipitation. The kinetic calculation therefore establishes that
entombment is \emph{feasible} on the timescale required, but the outcome of the entombment--decay
race depends critically on the surface chemistry of the cells and the degree of local
supersaturation. This sensitivity to surface properties, which varies with organism and environment,
provides a mechanistic explanation for the facies-dependent variability in preservation quality
across Precambrian cherts (Fig.~\ref{fig:kinetic}).

\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig_v10_kinetic.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.80\linewidth]{fig_gunflint.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{fig_v6_timeline.png}
...
The meta-analysis shows that silicification is the dominant predictor of cellular fidelity across
the Precambrian microfossil record ($\rho = 0.87$, $p < 0.001$), but that a significant
GOE-associated residual signal in fidelity persists after controlling for silicification (partial
$r = 0.72$, $p = 0.006$). The defensible position is therefore narrower than a claim that
silicification alone explains the post-GOE pattern. The preservation of cellular morphology in
Precambrian cherts is conditioned by early mineralisation, predominantly silicification, that
...
these limitations undermines the principal finding --- that silicification is the dominant predictor
--- but they do bound the strength of the claim about the residual GOE signal, whose biological
...
A systematic compilation of 15 Precambrian chert-hosted microfossil biotas shows that silicification
is the dominant predictor of cellular fidelity across the record (Spearman $\rho = 0.87$,
$p < 0.001$). A permutation test ($p = 0.0006$) and a perturbation analysis confirm that the
\emph{direction} of this association is robust: with $\pm$1-point noise on all scores, the
correlation remains positive in 95.2\% of 10{,}000 perturbations (mean perturbed $\rho = 0.40$).
The observed $\rho = 0.87$ should, however, be treated as an upper bound, inflated by clean coding,
rather than a point estimate. GOE position retains significant explanatory power after controlling
for silicification (partial $r = 0.72$, $p = 0.006$), positive in 95.4\% of perturbations. An
explicit kinetic calculation using the \citet{Rimstidt1980} rate law shows that abiotic silica
precipitates a 100~nm entombment layer in $\approx$1.3~hours under Precambrian silica saturation
($\Omega \approx 1.8$), one to two orders of magnitude faster than anoxic decay (3--14 days),
establishing feasibility with quantified uncertainty from surface catalysis and inhibition. We
conclude that silicification is a dominant taphonomic confound in the apparent post-GOE
diversification of microfossils, but that a significant GOE-associated residual signal in fidelity
persists that cannot be attributed to silicification alone. Whether this residual reflects genuine
biological diversification, an unmeasured preservational change, or sampling bias remains unresolved
by the meta-analysis. The biomarker evidence for pre-GOE eukaryotes is contaminated, but the
obligate aerobicity of sterol biosynthesis gives eukaryotes a mechanistically independent reason to
diversify with oxygen --- one candidate biological interpretation of the residual. We state the two
conditions that would falsify the silicification hypothesis and specify the experimental and
analytical programme needed to test them, one element of which is achievable on existing material.
